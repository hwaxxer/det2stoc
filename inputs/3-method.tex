...
\begin{tikzpicture}[shorten >=1pt,->,draw=black!50, node distance=\layersep and \nodesep, myarrow/.style={-Stealth}]
    ...
    \node[hidden neuron, right of=I-1, yshift=-0.5*\layersep] (he-1) {};
    \node[hidden neuron, below of=he-1] (he-2) {};
    \node[hidden neuron, below of=he-2] (he-3) {};
        
    \node[hidden neuron, right of=he-1, yshift=-.5*\layersep] (mu) {$\mu$};
    \node[hidden neuron, below of=mu] (sigma) {$\sigma$};
    ...
    \node[hidden neuron, right of=I-1, xshift=4*\layersep] (O-1) {$\mu_1$};
    \node[hidden neuron, below of=O-1] (O-2) {$\mu_2$};
    \node[hidden neuron, below of=O-2] (O-3) {$\sigma_1$};
    \node[hidden neuron, below of=O-3] (O-4) {$\sigma_2$};
    ...
    \begin{scope}[on background layer]
        ...
        \draw[rounded corners=3pt,draw=rose!100,ultra thick,dotted]
            ($(psi-1.north west)+(-0.3,0.7)$) rectangle ($(O-4.south east)+(2.1,-0.6)$);
