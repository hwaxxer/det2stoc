...
\begin{tikzpicture}[shorten >=1pt,->,draw=black!50, node distance=\nodesep and \layersep, myarrow/.style={-Stealth}]
    ...
    \node[hidden neuron, right=of he-1, yshift=-0.5*(\nodesize+\nodesep)] (mu) {$\mu$};
    ...
    \foreach \name in {1,...,3}
        \node[hidden neuron, right=of he-\name, xshift=2*\layersep + 2*\nodesize] (hd-\name) {$h^{(d)}_\name$};

    \node[hidden neuron, right=of I-1, xshift=4*\layersep + 4*\nodesize] (O-1) {$\mu_1$};
    ...
    \begin{scope}[on background layer]
        ...
        \draw[rounded corners=3pt,fill=curry!50]
            ($(he-1.north west)+(-0.5,0.4)$) rectangle ($(sigma.south east)+(0.4,-1.2)$);
        \draw[rounded corners=3pt,fill=moss!50]
            ($(hd-1.north west)+(-0.5,1.2)$) rectangle ($(O-4.south east)+(0.5,-0.5)$);
        \draw[rounded corners=3pt,draw=rose!100,ultra thick,dotted]
            ($(psi-1.north west)+(-0.3,0.7)$) rectangle ($(O-4.south east)+(2,-0.7)$);
