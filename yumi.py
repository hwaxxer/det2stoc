...
class YuMi(gym.GoalEnv):
    ...
    def observation_space(self):
        ...
        #return spaces.Box(low=-np.inf, high=np.inf, shape=(94,), dtype=np.float32)
    ...
    def callback(self, state):
        ...
        pids = np.take(self.pids, np.argsort(self.joint_idx))
        ...
        if VELOCITY_CONTROLLER:
            action = np.empty(nu)
            for i,j in enumerate(np.argsort(self.joint_idx)):
                pid = pids[j]
                qpos = data.qpos[i]
            ...
            #action = data.userdata[np.argsort(self.joint_idx)]
            for i,j in enumerate(np.argsort(self.joint_idx)):
                pid = pids[j]
                qpos = data.qpos[i]
                action[i] = pid(qpos, data.time)

            #res = action - data.qpos[:nu]
            res = action
            res -= data.qvel[:nu] * dt
    ...
    def reset(self):
        ...
        for i in range(self.model.nu):
            pid = PID(Kp=1.0, Kd=0.000, sample_time=0)
    ...
    def set_initial_pose(self):
        ...
        #qpos[0:2] += np.random.uniform(-0.05, 0.05, size=(2,))
    ...
    def render(self):
        if self.should_render:
            if self.sim.nsubsteps == self.steps_per_action and self.steps % 10 != 0:
                return
    ...
    def step(self, action):
        ...
        action = alpha * self.joint_states_vel + (1-alpha) * action
        #action += self.joint_states_vel#self.data.qvel[(self.joint_idx)]#self.joint_idx]#self.joint_states_pos
        #action = np.clip(action, self.joint_limits.low, self.joint_limits.high)
    ...
    def get_reward(self, distance, close=0.035, margin=0.14):
        # sigmoidal gaussian
        reward = 0 
